...
const AppointmentFormModal = ({ isVisible, onCancel, onSubmit, selectedAppointment }) => {
	...
	const { products, fetchAllProducts, loading: productsLoadingFromStore } = useProductStore();
	...
	useEffect(() => {
		const loadProducts = async () => {
			...
			try {
				...
				console.error("Error fetching products in AppointmentFormModal:", error);
			}
		};
	...
	useEffect(() => {
		if (isVisible) {
			...
			if (selectedAppointment) {
				form.setFieldsValue({
					...selectedAppointment,
					appointmentDateTime: selectedAppointment.appointmentDateTime ? moment(selectedAppointment.appointmentDateTime) : null,
					startTime: selectedAppointment.startTime ? moment(selectedAppointment.startTime) : null,
					endTime: selectedAppointment.endTime ? moment(selectedAppointment.endTime) : null,
					patientId: selectedAppointment.patientId,
					userId: selectedAppointment.userId,
					productId: selectedAppointment.productId,
				});
				// If editing, pre-populate search selects if data is available
				if (selectedAppointment.patientId && selectedAppointment.patientFirstName && selectedAppointment.patientLastName) {
					setPatientOptions([
						{
				...
				if (selectedAppointment.userId && selectedAppointment.userFirstName && selectedAppointment.userLastName) {
					setUserOptions([
						{
	...
	const handleFormSubmit = async () => {
		...
		try {
			...
			if (errorInfo.name === "SubmitError") {
				...
				console.error("Submission error handled by parent or service:", errorInfo.message);
				...
				// Other errors
				console.error("An unexpected error occurred during form submission:", errorInfo);
		...
		>
			<Form form={form} layout="vertical" name="appointment_form">
				...
				<Form.Item
					...
					<Select
						placeholder={t("appointments.formModal.placeholder.selectAppointmentType")}
